...
db.transaction{
  rows = db.execute("select * from illrecord where illnum = \"#{illnum}\";") {|row| 
    ...
    <body>
		...
		<input type=\"hidden\" name=\"illnum\" value=\"#{row[0]}\">
    ...
    puts "\t</select><input type=\"hidden\" name=\"illnum\" value=\"#{row[0]}\">\n\t<input type=\"submit\" value=\"図書館登録\">\n\t</form>"
    ...
    db.execute("select * from illstatus where illnum = \"#{row[0]}\";"){| status |
      printf(<<-STATUS, status[1], status[2], status[4])
    ...
    puts "<p><a href=\"#{ndl}\">NDL-OPAC</a></p>"
    puts "<p><a href=\"#{iss}\">国立国会図書館サーチ</a></p>"
  }
}
